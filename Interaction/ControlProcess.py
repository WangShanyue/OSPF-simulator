...
class MainProcess(Process):
    ...
    def setVal(self,delay):
        self.DelayTime=delay
        self.start()
    def run(self):
        print("延时时间为",self.DelayTime)
        ...
        for i in range(5):
            self.process_list.append(Socket.SubProcesses.RoutePrecess(i, 'Router{num}'.format(num=i), route[i],self.DelayTime))
        for i in range(5):
